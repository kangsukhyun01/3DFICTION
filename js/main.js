...
// 1) setInterval을 변수에 할달 //let 변수 = setInterval을
// 2) clearInterval (변수)

progressTimer=setInterval(updateProgress,450/60)
...
function updateProgress(){
    ...
    progressText.innerHTML=Math.floor(current) + '%' //Math.ceil(올림) Math.floor(버림)
    console.log(current)
...
function canvas() {
    ...
    canvas.width = window.innerWidth //viewport의 넓이를 canvas의 넓이값으로
    canvas.height = window.innerHeight //viewport의 높이를 canvas의 넓이값으로
    ...
    gsap.to(imageSeq,{
        ...
        snap:'frame', //frame은 프레임 단위로 값을 맞추겠다는 의미
        ease:'none',
        scrollTrigger:{
            scrub:0.15,
            trigger:'#page canvas',
        ...
        onUpdate:render //gsap.to가 변할때마다
    })
    ...
    function scaleImage(img, ctx){
        ...
        //max(값1, 값2) 값1과 값2중 큰값을 찾는다.
        //min(값1, 값2)둘중 작은것을 찾는다.
